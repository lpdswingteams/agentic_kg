...
from .tools import analyze_layout, extract_pdf_text, ocr_image_to_text, sniff_unstructured_text
...
class AgenticGovernanceAgent:
    ...
    def _build_system_prompt(self, strategies: Sequence[ChunkingStrategyConfig]) -> str:
        ...
        return (
            "你是知识治理智能体，负责自主规划。\n"
            "你可以调用提供的工具来查看 PDF 文本、执行 OCR、做版面分析或快速预览。\n"
    ...
    def plan(
        ...
        if isinstance(final_message, AIMessage):
            if getattr(final_message, "parsed", None):
                return PlanningResponse.model_validate(final_message.parsed)
            if isinstance(final_message.content, dict):
                return PlanningResponse.model_validate(final_message.content)
            if isinstance(final_message.content, str):
                return PlanningResponse.model_validate_json(final_message.content)

        content = getattr(final_message, "content", "")
        if isinstance(content, dict):
            return PlanningResponse.model_validate(content)
        if isinstance(content, str):
            return PlanningResponse.model_validate_json(content)

        raise ValueError("Agent response was not parseable into PlanningResponse")
